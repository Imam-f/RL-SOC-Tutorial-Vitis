...
//------------------------Parameter----------------------
`define TV_IN_readRep "../tv/cdatafile/c.VecDotProduct.autotvin_readRep.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter readRep_DEPTH = 1;
reg [31 : 0] readRep_OPERATE_DEPTH = 0;
parameter readRep_c_bitwidth = 32;
parameter readRep_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_readRep [readRep_DEPTH - 1 : 0];
reg readRep_write_data_finish;
...
//write readRep reg
reg [31 : 0] write_readRep_count = 0;
reg write_readRep_run_flag = 0;
reg write_one_readRep_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & readRep_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        readRep_write_data_finish <= 0;
        write_readRep_run_flag <= 0; 
        write_readRep_count = 0;
        count_operate_depth_by_bitwidth_and_depth (readRep_c_bitwidth, readRep_DEPTH, readRep_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_readRep_run_flag <= 1; 
            write_readRep_count = 0;
        end
        if (write_one_readRep_data_done === 1) begin
            write_readRep_count = write_readRep_count + 1;
            if (write_readRep_count == readRep_OPERATE_DEPTH) begin
                write_readRep_run_flag <= 0; 
                readRep_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_readRep
    integer write_readRep_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] readRep_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = readRep_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_readRep_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write readRep data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (readRep_c_bitwidth < 32) begin
                        readRep_data_tmp_reg = mem_readRep[write_readRep_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < readRep_c_bitwidth) begin
                                readRep_data_tmp_reg[j] = mem_readRep[write_readRep_count][i*32 + j];
                            end
                            else begin
                                readRep_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (readRep_data_in_addr + write_readRep_count * four_byte_num * 4 + i * 4, readRep_data_tmp_reg, write_readRep_resp);
                end
                process_busy = 0;
                write_one_readRep_data_done <= 1;
                @(posedge clk);
                write_one_readRep_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_readRep_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [readRep_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (readRep_c_bitwidth , factor);
  fp = $fopen(`TV_IN_readRep ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_readRep); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < readRep_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
